...
describe('ListMembers.vue', () => {
    ...
    it('fills the input, clicks the button, and checks if the store is updated', async () => {
      ...
      console.log(`Number of <tr> elements with data-testid="members": ${trElements.length}`);
      expect(trElements.length).toBeGreaterThanOrEqual(0); // Adjust this value based on your expectation
    });
  });
